			...
			<div id="post-<?php the_ID(); ?>" <?php post_class('post'); ?>>
				<div class="single_post">
					...
					<header>
						...
						<!-- End Title -->
						<div class="post-date-customizable">Posted On <?php the_time( get_option( 'date_format' ) ); ?></div>

					</header>

						<?php the_content(); ?>
						<?php wp_link_pages(array('before' => '<div class="pagination">', 'after' => '</div>', 'link_before'  => '<span class="current"><span class="currenttext">', 'link_after' => '</span></span>', 'next_or_number' => 'next_and_number', 'nextpagelink' => __('Next', 'customizable-blogily' ), 'previouspagelink' => __('Previous', 'customizable-blogily' ), 'pagelink' => '%','echo' => 1 )); ?>
						<?php if($customizable_blogily_single_tags_section == '1') { ?>
						<!-- Start Tags -->
						<div class="tags"><?php the_tags('<span class="tagtext">'.__('Tags','customizable-blogily').':</span>',', ') ?></div>
						<!-- End Tags -->
						<?php } ?>
					</div><!-- End Content -->
					...
					if ($categories) { $category_ids = array(); 
						...
						$my_query = new wp_query( $args ); if( $my_query->have_posts() ) {
							echo '<div class="related-posts"><div class="postauthor-top"><h3>'.esc_html__('Related Posts', 'customizable-blogily').'</h3></div><div class="related-posts-wrapper">';
							...
							while( $my_query->have_posts() ) { 
								...
								<div class="postauthor">
									<h4><?php esc_html_e('About The Author', 'customizable-blogily'); ?></h4>
